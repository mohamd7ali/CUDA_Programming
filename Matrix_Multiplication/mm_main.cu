...
void gpuKernel(const float* const a, const float* const b, float* c, const int m, const int n, double* gpu_kernel_time) {
	...
	dim3 dimGrid = getDimGrid(m,n); //modify this function in mm.cu
	dim3 dimBlock = getDimBlock(m,n); //modify this function in mm.cu

	GpuTimer timer;
    timer.Start();
	kernelFunc<<< dimGrid,dimBlock >>>(ad, bd, cd, m, n); //modify this function in mm.cu
	timer.Stop();
